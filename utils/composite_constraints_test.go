...
`
	...
	for _, tc := range cases {
		...
		if !ok {
			t.Fatalf("user.address 字段类型错误: %T", user["address"])
		}
...
`
	...
	for _, tc := range cases {
		...
		if !ok {
			t.Fatalf("addresses 字段类型错误: %T", tc["addresses"])
		}
		if len(arr) != 2 {
			t.Fatalf("addresses 长度错误: 期望2, 实际%d", len(arr))
		}
		a0, ok0 := arr[0].(map[string]any)
		a1, ok1 := arr[1].(map[string]any)
		if !ok0 || !ok1 {
			t.Fatalf("addresses 元素类型错误")
		}
		p0, _ := a0["province"].(string)
		c0, _ := a0["city"].(string)
		p1, _ := a1["province"].(string)
		c1, _ := a1["city"].(string)

		if !validPairs[[2]string{p0, c0}] {
			t.Fatalf("组合约束未按同组生成(数组-0): %s %s", p0, c0)
		}
		if !validPairs[[2]string{p1, c1}] {
			t.Fatalf("组合约束未按同组生成(数组-1): %s %s", p1, c1)
		}
		// 同一用例内，当前实现为单组覆盖，两个元素应一致
		if p0 != p1 || c0 != c1 {
			t.Fatalf("同一用例内数组元素不一致: (%s,%s) vs (%s,%s)", p0, c0, p1, c1)
		}
	}
}
