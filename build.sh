...
CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -o "$OUTPUT_DIR/atc_windows_386.exe" -ldflags="-s -w" .
if [ $? -eq 0 ]; then
...
CGO_ENABLED=0 GOOS=linux GOARCH=386 go build -o "$OUTPUT_DIR/atc_linux_386" -ldflags="-s -w" .
if [ $? -eq 0 ]; then
...
# Windows 压缩包
zip "atc_windows_386_v$VERSION.zip" "atc_windows_386.exe"
...
# Linux x86 压缩包
zip "atc_linux_386_v$VERSION.zip" "atc_linux_386"

cd ..

echo "构建完成！所有版本已保存到 $OUTPUT_DIR 目录"
echo "Windows x86: $OUTPUT_DIR/atc_windows_386_v$VERSION.zip"
echo "macOS ARM: $OUTPUT_DIR/atc_darwin_arm64_v$VERSION.zip"
echo "Linux ARM: $OUTPUT_DIR/atc_linux_arm64_v$VERSION.zip"
echo "Linux x86: $OUTPUT_DIR/atc_linux_386_v$VERSION.zip"