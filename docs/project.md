...
- **压测模式**：支持持续时间压力测试，提供详细的性能统计报告

#### 2.2.1 普通批量请求模式

普通批量请求模式会按顺序执行CSV文件中的所有测试用例一次，适用于功能测试、回归测试等场景。
...
压测模式通过指定`--duration`参数（或配置文件中的`duration`字段），可以对目标接口进行持续时间的压力测试。压测模式会循环使用CSV文件中的所有测试用例，持续发送请求直到达到指定的时长，并输出详细的性能统计报告。

**压测模式特性：**

1. **持续时间控制**：
   - 支持通过`--duration`或`-d`参数指定压测持续时间
   - 支持配置文件中的`[request].duration`字段配置压测时长
   - 支持s/m/h时间单位（秒/分钟/小时）
...
5. **详细报告**：
   压测结束后自动输出详细的性能统计报告，包括：
   - **请求统计**：总请求数、成功请求数、失败请求数、成功率
   - **性能指标**：QPS（每秒请求数）、实际耗时、预期时长
   - **响应时间统计**：
     - 平均响应时间（ms）
     - 最小响应时间（ms）
     - 最大响应时间（ms）
     - P90百分位数（90%的请求响应时间在此值以下）
     - P95百分位数（95%的请求响应时间在此值以下）
     - P99百分位数（99%的请求响应时间在此值以下）

6. **使用场景**：
   - 性能测试：评估接口的性能表现
   - 负载测试：测试系统在高负载下的稳定性
   - 压力测试：找出系统的性能瓶颈
   - 接口稳定性测试：验证接口长时间运行的稳定性

**使用示例：**

```shell
# 使用命令行参数指定压测时长30秒
atc request -u https://api.example.com/test -m post -f test_cases.csv -d 30s

# 压测5分钟，并发数设置为10
atc request -u https://api.example.com/test -m post -f test_cases.csv -d 5m -C 10

# 压测1小时，使用配置文件中的其他参数
atc request -c config.toml -d 1h

# 从配置文件读取压测时长（配置文件中设置 duration = "30s"）
atc request -c config.toml

# 压测模式组合使用鉴权和自定义头
atc request -u https://api.example.com/test -m post -f test_cases.csv \
  -d 2m --auth-bearer \"your_token\" --header \"X-Request-ID: bench001\"
```
...
并发数: 3
测试用例数: 5
目标URL: https://api.example.com/test
请求方法: POST
...
1. **结果不保存**：压测模式下不会将结果保存到CSV文件（忽略`save_path`参数）
2. **资源占用**：高并发压测会占用较多系统资源，请根据实际情况设置并发数
3. **时长控制**：压测会在达到指定时长后自动停止，实际运行时间可能略长于指定时长
4. **测试环境**：建议在测试环境进行压测，避免对生产环境造成影响
5. **测试用例准备**：确保CSV文件中包含足够的测试用例，以保证压测的多样性
...
timeout = 30
concurrent = 3
duration = "30s"  # 压测持续时间，支持s/m/h单位（如：30s, 5m, 1h），指定后进入压测模式
