...
- response_mode (string) Required 返回响应模式，支持：
    - treaming 流式模式（推荐）。基于 SSE（Server-Sent Events）实现类似打字机输出方式的流式返回。
...
- event: workflow_finished workflow 执行结束，成功失败同一事件中不同状态
    ...
    - data (object) 详细内容
        ...
        - elapsed_time (float) Optional 耗时(s)
        - total_tokens (int) Optional 总使用 tokens
        - total_steps (int) Optional 总步数（冗余），默认 0
        - created_at (timestamp) 开始时间
        - finished_at (timestamp) 结束时间
...
- file(Type: file): 要上传的文件。
- user(Type: string):用户标识，用于定义终端用户的身份，必须和发送消息接口传入 user 保持一致。服务 API 不会共享 WebApp 创建的对话。
...
- status(Type: string): 执行状态 succeeded/failed/stopped
- page(Type: int): 当前页码, 默认1.
- limit(Type: int): 每页条数, 默认20.
- created_by_end_user_session_id(Type: str): 由哪个endUser创建，例如，abc-123.
- created_by_account(Type: str): 由哪个帐号创建.

## Response
- page (Type: int) 当前页码
- limit (Type: int) 每页条数
- total (Type: int) 总条数
- has_more (Type: bool) 是否还有更多数据
- data (Type: array[object]) 当前页码的数据
  - id (Type: string) 标识
  - workflow_run (Type: object) Workflow 执行日志
    - id (Type: string) 标识
    - version (Type: string) 版本
    - status (Type: string) 执行状态，running / succeeded / failed / stopped
    - error (Type: string) (可选) 错误
    - elapsed_time (Type: float) 耗时，单位秒
    - total_tokens (Type: int) 消耗的 token 数量
    - total_steps (Type: int) 执行步骤长度
    - created_at (Type: timestamp) 开始时间
    - finished_at (Type: timestamp) 结束时间
  - created_from (Type: string) 来源
  - created_by_role (Type: string) 角色
  - created_by_account (Type: string) (可选) 帐号
  - created_by_end_user (Type: object) 用户
    - id (Type: string) 标识
    - type (Type: string) 类型
    - is_anonymous (Type: bool) 是否匿名
    - session_id (Type:string) 会话标识
  - created_at (Type: timestamp) 创建时间
